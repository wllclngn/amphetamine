...
// SAFETY: MessageRing lives in memfd-backed shared memory. The ring buffer uses
// atomic Acquire/Release on write_pos and read_pos for producer/consumer sync.
// UnsafeCell<[QueuedMessage; N]> is accessed only at indices guarded by these
// atomics — no aliased mutable access possible under SPSC discipline.
...
pub struct ThreadQueue {
    ...
    // Acquired by Wine client C code (triskelion_shm.h: triskelion_queue_lock/unlock)
    // before calling ring_push. The daemon never acquires this lock — it only reads
    // via pop() on the consumer side, which is wait-free (single consumer).
...
// SAFETY: ThreadQueue is #[repr(C)] with layout matching triskelion_shm.h.
// All shared fields are atomic (wake_bits, post_lock, MessageRing positions).
// thread_id is written once during init_at and read-only thereafter.
// post_lock is a CAS spinlock for multi-producer PostMessage (Wine clients).
...
mod tests {
    ...
    // ── ABI tests ────────────────────────────────────────────────────────────
    // These verify that struct sizes and alignments match the C layout contract.
    // If any of these fail, Wine clients reading/writing shared memory will corrupt data.

    #[test]
    fn queued_message_size() {
        assert_eq!(std::mem::size_of::<QueuedMessage>(), 48);
    }

    #[test]
    fn cache_line_u64_size_and_align() {
        assert_eq!(std::mem::size_of::<CacheLineU64>(), 64);
        assert_eq!(std::mem::align_of::<CacheLineU64>(), 64);
    }

    #[test]
    fn thread_queue_size_and_align() {
        assert_eq!(std::mem::size_of::<ThreadQueue>(), 12480);
        assert_eq!(std::mem::align_of::<ThreadQueue>(), 64);
    }

    #[test]
    fn message_ring_size() {
        // 2 * CacheLineU64 (128) + 256 * QueuedMessage (48) = 12416
        assert_eq!(std::mem::size_of::<MessageRing>(), 128 + 48 * RING_CAPACITY);
    }

    // ── Ring buffer correctness ──────────────────────────────────────────────

    fn zero_msg() -> QueuedMessage {
        QueuedMessage { win: 0, msg: 0, wparam: 0, lparam: 0, msg_type: 0, x: 0, y: 0, time: 0, _pad: [0; 2] }
    }

    fn make_ring() -> MessageRing {
        MessageRing {
            write_pos: CacheLineU64 { val: AtomicU64::new(0), _pad: [0; 56] },
            read_pos: CacheLineU64 { val: AtomicU64::new(0), _pad: [0; 56] },
            buf: UnsafeCell::new([zero_msg(); RING_CAPACITY]),
        }
    }
    ...
    fn ring_push_pop() {
        ...
        let msg = QueuedMessage { win: 100, msg: 42, wparam: 1, lparam: 2, msg_type: 1, ..zero_msg() };
        assert!(ring.push(msg));

        assert!(!ring.is_empty());
        let got = ring.pop().unwrap();
        assert_eq!(got.msg, 42);
        assert_eq!(got.win, 100);
        assert!(ring.is_empty());
    }
    ...
    fn ring_full() {
        ...
        for i in 0..RING_CAPACITY {
            let msg = QueuedMessage { msg: i as u32, ..zero_msg() };
            assert!(ring.push(msg), "push {i} should succeed");
        }
        // Next push should fail (ring full)
        let msg = QueuedMessage { msg: 999, ..zero_msg() };
        assert!(!ring.push(msg), "push to full ring should fail");
    }
    ...
    fn ring_wraparound() {
        ...
        // Push and pop to advance positions past the buffer size
        for round in 0..3u32 {
            for i in 0..RING_CAPACITY {
                let val = round * RING_CAPACITY as u32 + i as u32;
                assert!(ring.push(QueuedMessage { msg: val, ..zero_msg() }));
            }
            for i in 0..RING_CAPACITY {
                let val = round * RING_CAPACITY as u32 + i as u32;
                let got = ring.pop().unwrap();
                assert_eq!(got.msg, val);
            }
            assert!(ring.is_empty());
        }
    }
