...
//
// v2 format: unified .amphetamine_cache (10,496 bytes)
...
impl IntelManager {
    ...
    fn resolve_cache_path() -> Option<PathBuf> {
        ...
        Some(compat_dir.join(".amphetamine_cache"))
    }
    ...
    fn load(&mut self) {
        ...
        if let Ok(data) = std::fs::read(&v1_path) {
            if data.len() >= V1_FILE_SIZE && data[0..4] == V1_MAGIC {
                self.load_v1(&data);
                log_info!("intel: migrated v1 .triskelion_learned → v2 .amphetamine_cache");
            }
        }
    }
