...
impl EventLoop {
    ...
    pub(crate) fn handle_get_message(&mut self, client_fd: i32, buf: &[u8]) -> Reply {
        ...
        // No messages — reset queue event, return PENDING
        if let Some(client) = self.clients.get(&(client_fd as RawFd)) {
        ...
        if let Some(fd) = unix_fd {
            ...
            if dup_fd >= 0 {
                ...
                if let Some(client) = self.clients.get_mut(&(client_fd as RawFd)) {
                    // Close previous queue_fd if any (can only be set once in stock)
                ...
                // Add to epoll so broker wakes when X11 events arrive
                let mut ev = libc::epoll_event {
                    events: libc::EPOLLIN as u32 | libc::EPOLLET as u32,
                    u64: dup_fd as u64,
                };
                unsafe {
                    libc::epoll_ctl(self.epoll_fd, libc::EPOLL_CTL_ADD, dup_fd, &mut ev);
                }

                // Signal the queue immediately — there may already be pending X11 events
                if let Some(tid) = self.clients.get(&(client_fd as RawFd)).map(|c| c.thread_id) {
                    self.set_queue_bits_for_tid(tid, 0x0001 | 0x0002 | 0x0004); // QS_KEY | QS_MOUSEMOVE | QS_MOUSEBUTTON
                }
            ...
            return reply_fixed(&ReplyHeader { error: 0xC0000008, reply_size: 0 }); // INVALID_HANDLE
        }
    ...
    pub(crate) fn handle_create_window(&mut self, client_fd: i32, buf: &[u8]) -> Reply {
        ...
        let class_loc = self.class_locators.get(&atom).copied().unwrap_or([0u8; 16]);
        ...
        if atom == DESKTOP_CLASS_ATOM && self.desktop_top_window == 0 {
            ...
            self.window_states.insert(self.desktop_msg_window, WindowState {
                ...
                needs_paint: false, window_text: Vec::new(),
            });
        }

        // Return the client_ptr that was stored during create_class.
        // Wine dereferences this to access class data — returning 0 causes a crash.
        ...
        let reply = CreateWindowReply {
            ...
            parent: reply_parent,
            owner: req.owner,
            extra: 0, // TODO: win_extra causes ACCESS_VIOLATION — investigate
            class_ptr,
        };
        reply_fixed(&reply)
    }
    ...
    pub(crate) fn handle_get_window_info(&mut self, client_fd: i32, buf: &[u8]) -> Reply {
    ...
    pub(crate) fn handle_set_window_info(&mut self, _client_fd: i32, buf: &[u8]) -> Reply {
        ...
        // Wine 11.4 uses offset-based window info set/get.
        // req.offset indicates which field, req.new_info is the new value.
        // We store the old value and return it in old_info.
        let old_info: u64 = if let Some(ws) = self.window_states.get_mut(&req.handle) {
            // Return old value based on offset, then set new value.
            // The exact offsets depend on Wine's window_shm_t layout — stub returns 0.
            0
        } else {
            0
        };
    ...
    pub(crate) fn handle_get_thread_input(&mut self, client_fd: i32, buf: &[u8]) -> Reply {
        ...
        let locator = if req_tid == 0 {
            self.clients.get(&(client_fd as RawFd))
                .map(|c| c.input_locator)
                .unwrap_or([0u8; 16])
        } else {
            // Find client with matching thread_id
            self.clients.values()
                .find(|c| c.thread_id == req_tid)
                .map(|c| c.input_locator)
                .unwrap_or_else(|| {
                    // Thread not found — return caller's input as fallback
                    self.clients.get(&(client_fd as RawFd))
                        .map(|c| c.input_locator)
                        .unwrap_or([0u8; 16])
                })
    ...
    pub(crate) fn handle_get_msg_queue(&mut self, client_fd: i32, _buf: &[u8]) -> Reply {
        let locator = self.clients.get(&(client_fd as RawFd))
            .map(|c| c.queue_locator)
            .unwrap_or([0u8; 16]);
        let id = u64::from_le_bytes([locator[0], locator[1], locator[2], locator[3],
                                     locator[4], locator[5], locator[6], locator[7]]);
        let offset = u64::from_le_bytes([locator[8], locator[9], locator[10], locator[11],
                                         locator[12], locator[13], locator[14], locator[15]]);
    ...
    pub(crate) fn handle_send_hardware_message(&mut self, _client_fd: i32, _buf: &[u8]) -> Reply {
