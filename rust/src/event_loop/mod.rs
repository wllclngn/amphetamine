...
/// Win32 timer (stock: server/queue.c struct timer)
struct WinTimer {
    when: Instant,       // next expiration time
    rate_ms: u32,        // interval in milliseconds
    win: u32,            // target window handle (0 = thread timer)
    msg: u32,            // WM_TIMER or WM_SYSTIMER
    id: u64,             // timer id (lparam_t)
    lparam: u64,         // callback proc
}
...
struct WindowState {
    ...
    visible_rect: [u8; 16],
    surface_rect: [u8; 16],
    // Paint flags from client (PAINT_HAS_SURFACE=1, PAINT_HAS_PIXEL_FORMAT=2, PAINT_HAS_LAYERED_SURFACE=4)
    paint_flags: u16,
...
pub struct EventLoop {
    ...
    // Win32 timers: per-thread pending + expired lists (stock: queue.c:96-105, 119-141)
    // Key: tid → list of timers. Pending are sorted by expiration time.
    win_timers_pending: FxHashMap<u32, Vec<WinTimer>>,
    win_timers_expired: FxHashMap<u32, Vec<WinTimer>>,
...
impl EventLoop {
    pub fn new(listener: Listener, signal_fd: RawFd, shm: ShmManager, protocol_remap: crate::protocol_remap::ProtocolRemap, user_sid: Vec<u8>, user_sid_str: &str) -> Self {
        ...
        ev.set_window_shm(top_offset, &desktop_rect, NTUSER_DPI_PER_MONITOR_AWARE);
        ev.window_states.insert(ev.desktop_top_window, WindowState {
            ...
            paint_flags: 0, needs_paint: false, window_text: Vec::new(),
        });

        let msg_offset = ev.next_shared_offset;
        ev.desktop_msg_window = ev.alloc_user_handle(NTUSER_OBJ_WINDOW, 0, 0);
        ev.set_window_shm(msg_offset, &[0u8; 16], NTUSER_DPI_PER_MONITOR_AWARE);
    ...
    pub(crate) fn set_queue_bits_for_tid(&mut self, tid: u32, bits: u32) {
        ...
        if let Some(client) = client {
            ...
            if queue_handle != 0 {
                ...
                // Signal the queue's ntsync event so MsgWaitForMultipleObjects unblocks.
                // The thread is blocked in Select on this handle's ntsync event.
                // event_set on auto-reset wakes exactly one waiter (the thread).
    ...
    /// Check pending Win32 timers for expiration. Moves expired timers to
    /// the expired list and sets QS_TIMER on the owning thread's queue.
    /// Called from broker Tick (every ~50ms). Stock: queue.c timer_callback.
    pub(crate) fn check_win_timers(&mut self) {
        let now = Instant::now();
        // Collect tids that have expired timers
        let tids: Vec<u32> = self.win_timers_pending.keys().copied().collect();
        for tid in tids {
            let mut newly_expired = Vec::new();
            if let Some(pending) = self.win_timers_pending.get_mut(&tid) {
                // Move expired timers out (when <= now)
                let mut i = 0;
                while i < pending.len() {
                    if pending[i].when <= now {
                        newly_expired.push(pending.remove(i));
                    } else {
                        i += 1;
                    }
                }
            ...
            if !newly_expired.is_empty() {
                ...
                self.set_queue_bits_for_tid(tid, QS_TIMER);
                self.win_timers_expired.entry(tid).or_default().extend(newly_expired);
            }
        }
    }

    /// Poll display driver queue fds for readability. When X11 events arrive
    /// on a client's queue_fd, set QS_INPUT wake bits so get_message unblocks.
    /// Called from broker Tick (every ~50ms).
    pub(crate) fn poll_queue_fds(&mut self) {
        // Collect (tid, fd) pairs to avoid borrow issues
        let fds: Vec<(u32, i32)> = self.clients.values()
            .filter_map(|c| c.queue_fd.map(|fd| (c.thread_id, fd)))
            .collect();

        for (tid, fd) in fds {
            // Non-blocking poll: check if fd has data ready
            let mut pfd = libc::pollfd { fd, events: libc::POLLIN, revents: 0 };
            let ready = unsafe { libc::poll(&mut pfd, 1, 0) };
            if ready > 0 && pfd.revents & libc::POLLIN != 0 {
                // X11 events available — wake the thread's message queue
                const QS_INPUT: u32 = 0x0001 | 0x0002 | 0x0004; // KEY | MOUSEMOVE | MOUSEBUTTON
                self.set_queue_bits_for_tid(tid, QS_INPUT);
            }
        }
    }
...
/// Default screen resolution. The real resolution comes from winex11.drv
/// via set_winstation_monitors at runtime, overwriting the desktop rect.
