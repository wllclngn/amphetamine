        ...
        match resolved {
            Some(code) => {
                ...
                if err == 0xC000_0002 {
                    ...
                    if !is_esync_fsync {
                        self.intel.record_stub(idx);
                        if self.intel.should_auto_stub(idx, meta.has_vararg_reply) {
                            // WARNING: auto-stub zeroes ALL reply fields including handles.
                            // Any reply with a handle field gets handle=0 with SUCCESS.
                            // Wine stores the NULL handle and crashes later.
                            // TODO: return NOT_IMPLEMENTED instead for opcodes with handle replies.
                    ...
                    // else: esync/fsync — pass NOT_IMPLEMENTED through to client
                } else if err != 0 && err != 0x8000001a && err != 0xc0000034 {
