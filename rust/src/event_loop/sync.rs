...
impl EventLoop {
    ...
    // Send immediate wake_up_reply for Select results delivered inline.
    // Wine protocol: Select ALWAYS uses STATUS_PENDING on reply_fd +
    // wake_up_reply on wait_fd, even for immediate results.
    fn send_select_wake(&mut self, client_fd: RawFd, cookie: u64, signaled: i32) {
        if let Some(client) = self.clients.get(&client_fd) {
            if let Some(wait_fd) = client.wait_fd {
                ...
                if n != std::mem::size_of::<WakeUpReply>() as isize {
                    log_error!("BUG: send_select_wake fd={client_fd} wait_fd={wait_fd} write returned {n} (errno={})", std::io::Error::last_os_error());
                }
                // DO NOT signal the alert here. The alert is shared with inproc
                // ntsync waits (get_inproc_alert_fd). Signaling it for non-APC wakes
                // causes inproc waits to return ALERTED, triggering Wine's assertion
                // at sync.c:441 (expects STATUS_USER_APC but no APC is queued).
                // The write to wait_fd is sufficient for daemon-side wait threads.
            } else {
                // wait_fd not yet available. Two cases:
                // 1. Thread hasn't called init_first_thread yet -- no active Select,
                //    APC is already queued in pending_kernel_apcs, will be delivered
                //    on first Select. No action needed.
                // 2. Thread IS in a blocking ntsync wait but phantom's wait_fd wasn't
                //    set (race). Use alert path to interrupt the kernel wait. The wait
                //    thread exits, thread re-enters Select, finds the pending APC.
                // DO NOT signal alert — no APC queued. Signaling without APC
                // causes Wine's sync.c:441 assertion (expects STATUS_USER_APC).
                // Queue the wake for delivery when wait_fd becomes available
                self.pending_wakes.entry(client_fd).or_default().push((cookie, signaled));
            }
        } else {
            log_warn!("send_select_wake: fd={client_fd} client not found (may have disconnected)");
        }
    }
    ...
    pub(crate) fn handle_select(&mut self, client_fd: i32, buf: &[u8]) -> Reply {
        ...
        if has_contexts && req.size == 0 {
            ...
            if let Some(client) = self.clients.get_mut(&(client_fd as RawFd)) {
                client.suspend_cookie = req.cookie as u64;
            }
        ...
        // Wine protocol: Select ALWAYS returns STATUS_PENDING on reply_fd.
        // The actual result is ALWAYS delivered via wake_up_reply on wait_fd,
        // even for immediate results. The client enters wait_select_reply()
        // after every Select call.
        ...
        // STATUS_PENDING is sent by the worker (WorkerReply::Deferred path).
        // Don't write it here — double write corrupts the reply stream.

        // If we have an immediate result, send it via wake_up_reply on wait_fd
        if let Some(signaled) = immediate_result {
            self.send_select_wake(client_fd as RawFd, req.cookie, signaled);
        ...
        let duration = std::time::Duration::from_nanos(timeout_ns);
    ...
    pub(crate) fn handle_get_inproc_sync_fd(&mut self, client_fd: i32, buf: &[u8]) -> Reply {
        ...
        {
            let type_name = self.ntsync_objects.get(&(pid, handle))
                .map(|(_, st)| match *st { 1 => "INT", 2 => "EVT", 3 => "MUT", 4 => "SEM", _ => "?" })
                .unwrap_or("NONE");
            log_info!("GET_INPROC_SYNC: pid={pid} handle={handle:#x} type={type_name} fd={client_fd}");
        }
        ...
        // Handle not found — create on-demand manual-reset SIGNALED event.
        // Signaled so timeout-based waits complete instead of blocking forever.
