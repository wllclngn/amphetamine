...
///   2. ~/.local/share/amphetamine/protocols/<version>/protocol.def (cached)
...
/// Cache directory for protocol files: ~/.local/share/amphetamine/protocols/<version>/
fn cache_dir_for_version(version: u32) -> PathBuf {
    let home = std::env::var("HOME").unwrap_or_else(|_| "/tmp".to_string());
    PathBuf::from(home)
        .join(".local/share/amphetamine/protocols")
