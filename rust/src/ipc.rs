...
impl Listener {
    ...
    /// Switch listener to blocking mode (for acceptor thread).
    pub fn set_blocking(&self) {
        self.inner.set_nonblocking(false)
            .expect("set_blocking on listener");
    }

    /// Accept a new Wine client connection. Creates the request pipe and
    /// sends the write-end + protocol version to the client immediately.
    /// Returns (Client, msg_fd) where msg_fd is the connection socket
    /// that needs to be monitored for SCM_RIGHTS fd passing.
    pub fn accept(&self) -> Option<(Client, RawFd)> {
        match self.inner.accept() {
            Ok((stream, _)) => {
                ...
                if unsafe { libc::pipe2(request_pipe.as_mut_ptr(), libc::O_CLOEXEC) } != 0 {
                    log_error!("pipe2 failed: {}", std::io::Error::last_os_error());
                    unsafe { libc::close(msg_fd); }
                    return None;
                }

                // Send request_pipe[1] (write end) to client via SCM_RIGHTS.
                // The data payload MUST be the protocol version the client expects.
                // Wine's wine_server_receive_fd() stores this in `version`,
                // then checks `if (version != SERVER_PROTOCOL_VERSION)` →
                // "version mismatch" → process terminated.
                let send_result = send_fd(msg_fd, request_pipe[1], runtime_protocol_version());
                if send_result < 0 {
                    let err = std::io::Error::last_os_error();
                    log_error!("failed to send request_fd to client: {err}");
                    unsafe {
                        libc::close(msg_fd);
                ...
                // Without this, a worker still polling on a recycled fd number steals data.
                let safe_fd = unsafe { libc::dup(request_pipe[0]) };
                unsafe { libc::close(request_pipe[0]); }
                if safe_fd < 0 {
                    log_error!("accept: dup failed");
                    unsafe { libc::close(msg_fd); }
                    return None;
                }
                ...
                log_info!("accepted client: request_fd={safe_fd} msg_fd={msg_fd}");
                Some((Client::new(safe_fd, msg_fd), msg_fd))
            }
...
impl Client {
    ...
    /// Create a phantom (broker-side mirror) of this client.
    /// Shares fd values for handler compatibility but does NOT own them.
    /// Phantom Drop closes only msg_fd and unclaimed inflight_fds.
    pub fn phantom_from(real: &Client) -> Self {
        Self {
            fd: real.fd,
            msg_fd: real.msg_fd,
            reply_fd: real.reply_fd,
            process_id: real.process_id,
            thread_id: real.thread_id,
            suspend_cookie: real.suspend_cookie,
            current_wait_cookie: real.current_wait_cookie,
            recv_buf: Vec::new(),
            inflight_fds: VecDeque::new(),
            wait_fd: real.wait_fd,
            queue_locator: real.queue_locator,
            input_locator: real.input_locator,
            queue_handle: real.queue_handle,
            teb: real.teb,
            entry_point: real.entry_point,
            unix_pid: real.unix_pid,
            unix_tid: real.unix_tid,
            queue_fd: real.queue_fd,
            is_phantom: true,
            pending_fd: None,
        }
    }

    // Read protocol requests from the request pipe into internal buffer.
    // Plain read (no recvmsg needed -- pipes don't carry ancillary data).
    // Returns bytes read, 0 on disconnect, -1 on EAGAIN.
    #[inline]
    pub fn read_into_buf(&mut self) -> isize {
        let mut tmp = [0u8; 4096];
        let n = unsafe { libc::read(self.fd, tmp.as_mut_ptr() as *mut _, tmp.len()) };
        if n > 0 {
    ...
    // Read SCM_RIGHTS file descriptors from the connection socket (msg_fd).
    // Wine sends fds here via wine_server_send_fd(). Each message carries
    // a struct { thread_id: u32, fd_number: i32 } plus the actual fd.
    // Returns bytes read, 0 on disconnect, -1 on EAGAIN.
    pub fn read_fds_from_msg(&mut self) -> isize {
        let mut tmp = [0u8; 256];
        ...
        let n = unsafe { libc::recvmsg(self.msg_fd, &mut msg, 0) };

        if n > 0 {
            // Parse data payload: struct send_fd { thread_id: u32, fd_number: i32 }
            // Each SCM_RIGHTS message from wine_server_send_fd carries this 8-byte payload.
            let send_tid = if n >= 4 {
                u32::from_le_bytes([tmp[0], tmp[1], tmp[2], tmp[3]])
            } else { 0 };
            let client_fd_number = if n >= 8 {
                i32::from_le_bytes([tmp[4], tmp[5], tmp[6], tmp[7]])
            } else {
                -1 // No fd_number in payload (shouldn't happen)
            };
            // Store thread_id in the upper bits of client_fd_number tag.
            // We use a separate field: push (fd_number, actual_fd) as before,
            // but also record the thread_id for routing.
            ...
            while !cmsg.is_null() {
                ...
                if hdr.cmsg_level == libc::SOL_SOCKET && hdr.cmsg_type == libc::SCM_RIGHTS {
                    ...
                    for i in 0..fd_count {
                        ...
                        self.inflight_fds.push_back((send_tid, client_fd_number, fd));
                    }
                }
    ...
    // Check if a complete request is buffered. Wine sends 64 bytes (fixed
    // union) + request_size bytes (variable data) for every request.
    #[inline]
    pub fn has_complete_request(&self) -> bool {
        if self.recv_buf.len() < FIXED_REQUEST_SIZE {
            return false;
        }
        ...
        self.recv_buf.len() >= FIXED_REQUEST_SIZE + request_size
    }
    ...
    pub fn take_request(&mut self, out: &mut Vec<u8>) {
        ...
        let total = FIXED_REQUEST_SIZE + request_size;
        out.clear();
...
// Wine sends every request as sizeof(union generic_request) = 64 bytes (fixed
// part), followed by request_size bytes of variable-length data (VARARG).
// The union is padded by struct request_max_size { int pad[16]; }.
pub const FIXED_REQUEST_SIZE: usize = 64;

impl Drop for Client {
    fn drop(&mut self) {
        if self.is_phantom {
            // Phantom: broker owns msg_fd for SCM_RIGHTS reads/sends.
            // Worker owns fd (request_fd), reply_fd, wait_fd — don't touch them.
            // Do NOT close msg_fd here — it's shared between all threads of the
            // process. Closing it when any single thread disconnects would break
            // all sibling threads. msg_fd is closed by disconnect_client when the
            // last thread of the process dies.
            for &(_, _, fd) in &self.inflight_fds {
                unsafe { libc::close(fd); }
            }
            return;
        }
        // Real client: owns fd, reply_fd, wait_fd. msg_fd may be -1 (worker).
