...
impl DeployPlan {
    fn from_scan(scan: Option<&crate::pe_scanner::PeScanResult>) -> Self {
        ...
        let has_any_d3d = scan.imports.iter().any(|s|
...
pub fn run(verb: &str, args: &[String]) -> i32 {
    ...
    // Write to a file so we can always see it regardless of stderr redirection
    {
        ...
        let _ = std::fs::write("/tmp/amphetamine/launcher_env.txt", env_dump);
    }
    ...
    if verb == "waitforexitandrun" || verb == "run" {
        ...
        let _ = std::fs::create_dir_all("/tmp/amphetamine");
        let stderr_log_path = {
            let path = PathBuf::from("/tmp/amphetamine/wine_stderr.log");
            match std::fs::File::create(&path) {
                Ok(f) => {
                    cmd.stderr(std::process::Stdio::from(f));
                    if let Ok(f2) = std::fs::File::create("/tmp/amphetamine/wine_stdout.log") {
        ...
        // Spawn explorer.exe /desktop in the background.
        // Explorer creates the X11 desktop window and runs the display
        // management thread. Without it, windows are created in our daemon
        // but never mapped to X11 (no XMapWindow, no rendering).
        {
            let mut explorer_cmd = Command::new(&wine64);
            explorer_cmd.arg("explorer.exe");
            explorer_cmd.arg("/desktop");
            for (k, v) in &env_vars {
                explorer_cmd.env(k, v);
            }
            explorer_cmd.env_remove("WAYLAND_DISPLAY");
            explorer_cmd.env("WINEDLLOVERRIDES", "winewayland.drv=");
            explorer_cmd.stdout(std::process::Stdio::null());
            explorer_cmd.stderr(std::process::Stdio::null());
            match explorer_cmd.spawn() {
                Ok(_) => log_verbose!("Spawned explorer.exe /desktop (after game)"),
                Err(e) => log_warn!("Failed to spawn explorer.exe: {e}"),
            }
            // Wait for desktop_ready via atomic flag in SHM header.
            // No file I/O, no stale sentinels, no PID validation needed.
            // The daemon sets ShmHeader.desktop_ready = 1 when explorer creates the desktop.
            let prefix_hash = {
            ...
            let mut ready = false;
            let c_name = std::ffi::CString::new(shm_name.as_str()).unwrap_or_default();
            let shm_fd = unsafe { libc::shm_open(c_name.as_ptr(), libc::O_RDONLY, 0) };
            if shm_fd >= 0 {
                let ptr = unsafe {
                    libc::mmap(std::ptr::null_mut(), 64, libc::PROT_READ,
                               libc::MAP_SHARED, shm_fd, 0)
                };
                if ptr != libc::MAP_FAILED {
                    let flag = unsafe { &*((ptr as *const u8).add(20) as *const std::sync::atomic::AtomicU8) };
                    let deadline = std::time::Instant::now() + std::time::Duration::from_secs(30);
                    while !ready {
                        if flag.load(std::sync::atomic::Ordering::Acquire) != 0 {
                            ready = true;
                            break;
                        }
                        if std::time::Instant::now() > deadline {
                            log_warn!("Timed out waiting for desktop_ready (30s)");
                            break;
                        }
            ...
            if ready {
                log_verbose!("Desktop ready (atomic SHM signal)");
            }
        }
...
fn setup_prefix(wine_dir: &Path, pfx: &Path, wine64: &Path, self_exe: &Path) {
    ...
    if default_pfx.exists() {
        ...
        // Use stock wineserver for prefix creation — it creates the full
        // directory structure (Fonts, temp, winsxs, etc.) and registry entries
        // that games depend on. Our daemon handles the game session afterward.
        let stock_wineserver = Path::new("/usr/bin/wineserver");
        if stock_wineserver.exists() {
            cmd.env("WINESERVER", stock_wineserver.as_os_str());
            log_verbose!("Using stock wineserver for prefix init");
        } else {
        ...
        // Only suppress Wine debug if not already set (allows iterate.py to pass through)
        if std::env::var("WINEDEBUG").is_err() {
            cmd.env("WINEDEBUG", if crate::log::is_verbose() {
                "+process,+seh,err"
            } else {
                "-all"
            });
        ...
        let noassert = {
            ...
            let installed = PathBuf::from(std::env::var("HOME").unwrap_or_default())
                .join(".local/share/Steam/compatibilitytools.d/amphetamine/lib/noassert.so");
...
fn inject_registry_keys(pfx: &Path) {
    ...
    // ── Amphetamine-specific overrides ──
    // These are the ONLY keys we inject on top of the stock template.
    // Everything else (services, shell folders, VC++, .NET, etc.) comes
    // from the stock Wine prefix template copied above.

    // Force display driver to x11 (GLX over XWayland)
    {
        let contents = std::fs::read_to_string(&user_reg).unwrap_or_default();
        let driver_value = "\"Graphics\"=\"x11\"";
        if !contents.contains("Software\\\\Wine\\\\Drivers") {
            let keys = format!("\n[Software\\\\Wine\\\\Drivers] 1772204972\n#time=1dca7fb13d11a48\n{driver_value}\n\n");
            if let Ok(mut f) = std::fs::OpenOptions::new().append(true).open(&user_reg) {
                let _ = f.write_all(keys.as_bytes());
                log_verbose!("Registry: set display driver (x11)");
            }
        } else {
            let mut new_lines = Vec::new();
            for line in contents.lines() {
                if line.trim().starts_with("\"Graphics\"=") && !line.contains("GraphicsDriver") {
                    new_lines.push(driver_value.to_string());
                } else {
            ...
            if new_contents != contents {
                let _ = std::fs::write(&user_reg, &new_contents);
                log_verbose!("Registry: force-updated display driver (x11)");
            }
        }
    }

    // Force GraphicsDriver=winex11.drv in system.reg.
    // The stock ~/.wine template may have winewayland.drv on Wayland systems.
    // Wine reads this to decide which display driver to load.
    {
        let contents = std::fs::read_to_string(&sys_reg).unwrap_or_default();
        if contents.contains("\"GraphicsDriver\"=\"winewayland.drv\"") {
            let new_contents = contents.replace(
                "\"GraphicsDriver\"=\"winewayland.drv\"",
                "\"GraphicsDriver\"=\"winex11.drv\""
            );
            let _ = std::fs::write(&sys_reg, &new_contents);
            log_verbose!("Registry: force-updated GraphicsDriver to winex11.drv");
        }
    }
...
fn deploy_steam_exe(self_exe: &Path, pfx: &Path) {
    // Source our steam_bridge from the amphetamine lib directory (deployed by install.py).
...
fn build_env_vars(
    ...
    trace: bool, shader_cache_enabled: bool,
    ...
    // WINEDLLPATH: proton bridge DLLs → amphetamine libs → vkd3d → wine DLLs → Proton DLLs
    ...
    let proton_override = PathBuf::from(&home)
        .join(".local/share/Steam/compatibilitytools.d/amphetamine/lib/wine/proton");
    ...
    let amphetamine_wine_lib = self_exe.parent().map(|d| d.join("lib/wine"));
    if let Some(ref wlib) = amphetamine_wine_lib {
        if wlib.exists() {
    ...
    let mut vars = vec![
        ...
        ("WINEDEBUG", if trace { "+server,+timestamp".into() } else if crate::log::is_verbose() { "+process,+seh,err".into() } else { "-all".into() }),
        ("WINEDLLOVERRIDES", dll_overrides),
        ("DXVK_LOG_LEVEL", "none".into()),
        ("VKD3D_DEBUG", "none".into()),
        ("WINE_LARGE_ADDRESS_AWARE", "1".into()),
        ("PROTON_VERSION", format!("amphetamine {}", env!("CARGO_PKG_VERSION"))),
...
fn _sync_prefix_env(
    wine64: &Path, pfx: &Path, self_exe: &Path, wine_dir: &Path,
...
fn find_wine() -> PathBuf {
    ...
    // amphetamine's staged wine binaries — uses our patched ntdll.so
