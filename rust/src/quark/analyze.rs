...
use triskelion::{log_info, log_warn};
...
pub fn run() -> i32 {
    ...
    println!();
    log_info!("Code to keep:    {:>10} LOC", triskelion::log::format_with_commas(keep_loc));
    log_info!("Code to remove:  {:>10} LOC", triskelion::log::format_with_commas(remove_loc));
    ...
    for (dll, loc) in dll_sizes.iter().take(20) {
        println!("  {:<25} {:>8} LOC", dll, triskelion::log::format_with_commas(*loc));
    }
