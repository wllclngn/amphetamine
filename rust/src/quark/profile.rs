...
use triskelion::{log_info, log_warn, log_error};
...
fn generate_strace_summary(log_file: &Path, out_dir: &Path, prefix: &str) {
    ...
    log_info!("  Total syscalls: {}", triskelion::log::format_with_commas(total));
    log_info!("  Unique syscalls: {}", syscall_counts.len());
    log_info!("  Futex calls: {}", triskelion::log::format_with_commas(futex_count));
    log_info!("  Wineserver socket traffic: {}", triskelion::log::format_with_commas(socket_count));
    log_info!("  Wineserver RPCs: recvmsg={}, sendmsg={}",
        triskelion::log::format_with_commas(recvmsg_count),
        triskelion::log::format_with_commas(sendmsg_count));

    log_info!("  Top 10 syscalls:");
    for (syscall, count) in sorted.iter().take(10) {
        println!("    {:>10}  {syscall}", triskelion::log::format_with_commas(*count));
    }
}
...
pub fn run_profile_compare(dir_a: &str, dir_b: &str) -> i32 {
    ...
    for prefix in &["wineserver", "game"] {
        ...
        println!("    {:>20}  {:>10}  {:>10}  {:>10}",
            "TOTAL",
            triskelion::log::format_with_commas(total_a),
            triskelion::log::format_with_commas(total_b),
        ...
        for &sc in &key_syscalls {
            ...
            println!("    {:>20}  {:>10}  {:>10}  {:>10}",
                sc,
                triskelion::log::format_with_commas(va),
                triskelion::log::format_with_commas(vb),
                format_delta(va, vb));
        }
    }
...
//
// Parses traces captured with QUARK_TRACE_OPCODES=1 (which sets
...
pub fn run_profile_opcodes(trace_file: &str) -> i32 {
    ...
    if !path.exists() {
        ...
        log_error!("  touch /tmp/quark/TRACE_OPCODES && steam steam://rungameid/<app_id>");
        return 1;
    }
    ...
    log_info!("  Total requests: {}", triskelion::log::format_with_commas(total_calls));
    log_info!("  Total errors: {}", triskelion::log::format_with_commas(total_errors));
    log_info!("  Unique opcodes: {}", opcode_counts.len());
    log_info!("  Wine threads: {}", thread_ids.len());
    log_info!("  Trace lines: {}", triskelion::log::format_with_commas(line_number));
    ...
    for (opcode, count) in sorted.iter().take(30) {
        ...
        println!("  {:>8}  {:>5.1}%  {opcode}",
            triskelion::log::format_with_commas(*count), pct);
    }
    ...
    if !startup_counts.is_empty() || !steady_counts.is_empty() {
        ...
        log_info!("Phase breakdown (startup = first {} requests):",
            triskelion::log::format_with_commas(startup_call_limit));
        log_info!("  Startup: {} requests", triskelion::log::format_with_commas(startup_total));
        log_info!("  Steady-state: {} requests", triskelion::log::format_with_commas(steady_total));
        ...
        if !startup_only.is_empty() {
            ...
            for (opcode, count) in so.iter().take(20) {
                println!("    {:>6}  {opcode}", triskelion::log::format_with_commas(*count));
            }
        }
        ...
        if !steady_sorted.is_empty() {
            ...
            for (opcode, count) in steady_sorted.iter().take(20) {
                println!("    {:>8}  {:>5.1}%  {opcode}",
                    triskelion::log::format_with_commas(*count),
    ...
    if !error_counts.is_empty() {
        ...
        for (desc, count) in err_flat.iter().take(15) {
            println!("    {:>6}  {desc}", triskelion::log::format_with_commas(*count));
        }
    }
    ...
    let _ = fs::remove_file("/tmp/quark/TRACE_OPCODES");
