// amphetamine launcher — C23 port of rust/src/launcher.rs
//
...
#define LOG_INFO(fmt, ...)    fprintf(stderr, "[amphetamine] " fmt "\n" __VA_OPT__(,) __VA_ARGS__)
#define LOG_ERROR(fmt, ...)   fprintf(stderr, "[amphetamine] ERROR: " fmt "\n" __VA_OPT__(,) __VA_ARGS__)
#define LOG_WARN(fmt, ...)    fprintf(stderr, "[amphetamine] WARN: " fmt "\n" __VA_OPT__(,) __VA_ARGS__)
...
static void deploy_steam_exe(const char *proton_dir, const char *pfx) {
    ...
    snprintf(cached, sizeof(cached), "%s/.local/share/amphetamine/steam.exe", g_home);
...
static void find_wine(char *out, size_t outsize) {
    ...
    // 2. Amphetamine's staged wine (self_exe dir)
    ...
    bool shader_cache_enabled)
{
    ...
    snprintf(filename, sizeof(filename), "/tmp/amphetamine/launch-%s.prom", ts);
    ...
    fprintf(f, "# Generated by amphetamine launcher (C23)\n\n");
    fprintf(f, "# HELP amphetamine_system_info System identification\n");
    fprintf(f, "amphetamine_system_info{kernel=\"%s\",gpu_vendor=\"%s\"} 1\n", kernel, gpu);
    fprintf(f, "amphetamine_system_cpu_count %ld\n", cpus);
    fprintf(f, "amphetamine_system_ram_bytes %lu\n", (unsigned long)ram);

    // App info
    const char *app_id = strrchr(compat_data, '/');
    app_id = app_id ? app_id + 1 : compat_data;
    fprintf(f, "\namphetamine_steam_app_id{app_id=\"%s\"} 1\n", app_id);
    if (game_exe) fprintf(f, "amphetamine_game_executable{path=\"%s\"} 1\n", game_exe);

    // Paths
    fprintf(f, "\namphetamine_wine_dir{path=\"%s\"} 1\n", wine_dir);
    fprintf(f, "amphetamine_steam_dir{path=\"%s\"} 1\n", steam_dir);
    fprintf(f, "amphetamine_wineserver{path=\"%s\"} 1\n", self_exe);
    fprintf(f, "amphetamine_wineprefix{path=\"%s\"} 1\n", pfx);

    // Cache
    fprintf(f, "\namphetamine_cache_hit{component=\"wine\"} %d\n", wine_valid);
    fprintf(f, "amphetamine_cache_hit{component=\"dxvk\"} %d\n", dxvk_valid);
    fprintf(f, "amphetamine_cache_hit{component=\"vkd3d\"} %d\n", vkd3d_valid);
    fprintf(f, "amphetamine_cache_hit{component=\"steam\"} %d\n", steam_valid);

    // Timing
    fprintf(f, "\namphetamine_setup_duration_ms{phase=\"discover\"} %lu\n", (unsigned long)t_discover);
    fprintf(f, "amphetamine_setup_duration_ms{phase=\"prefix\"} %lu\n", (unsigned long)t_prefix);
    fprintf(f, "amphetamine_setup_duration_ms{phase=\"dxvk_vkd3d\"} %lu\n", (unsigned long)t_dxvk);
    fprintf(f, "amphetamine_setup_duration_ms{phase=\"steam_client\"} %lu\n", (unsigned long)t_steam);
    fprintf(f, "amphetamine_setup_duration_ms{phase=\"total\"} %lu\n", (unsigned long)t_total);

    // Shader cache
    fprintf(f, "\namphetamine_shader_cache_enabled %d\n", shader_cache_enabled);

    // Save data stats
    uint32_t save_files = 0;
    uint64_t save_bytes = 0;
    count_save_data_stats(pfx, &save_files, &save_bytes);
    fprintf(f, "\namphetamine_save_backup_files %u\n", save_files);
    fprintf(f, "amphetamine_save_backup_bytes %lu\n", (unsigned long)save_bytes);

    // Env vars
    fprintf(f, "\n");
    for (int i = 0; i < env->count; i++) {
        fprintf(f, "amphetamine_env_var{name=\"%s\"} 1\n", env->pairs[i].key);
    }

    fclose(f);

    // Latest symlink
    unlink("/tmp/amphetamine/launch-latest.prom");
    char basename[128];
    snprintf(basename, sizeof(basename), "launch-%s.prom", ts);
    symlink(basename, "/tmp/amphetamine/launch-latest.prom");
...
static int launcher_run(const char *verb, char *const *args, int nargs) {
    ...
    char dxvk_src_dir[PATH_MAX];
    {
        ...
        snprintf(amp_dxvk, sizeof(amp_dxvk),
            "%s/.local/share/Steam/compatibilitytools.d/amphetamine", g_home);
        char check[PATH_MAX];
    ...
    bool trace = getenv("AMPHETAMINE_TRACE_OPCODES") != nullptr
              || file_exists("/tmp/amphetamine/TRACE_OPCODES");
    ...
    if (strcmp(verb, "waitforexitandrun") == 0 || strcmp(verb, "run") == 0) {
        ...
        if (trace) {
            makedirs("/tmp/amphetamine");
            stderr_fd = open("/tmp/amphetamine/opcode_trace.log",
            ...
            snprintf(stderr_log, sizeof(stderr_log),
                "/tmp/amphetamine/stderr-%s.log", ts2);
        ...
        if (WIFEXITED(status)) {
            ...
            if (stderr_log[0]) {
                // Update symlink
                unlink("/tmp/amphetamine/stderr-latest.log");
                char *bn = strrchr(stderr_log, '/');
                if (bn) symlink(bn + 1, "/tmp/amphetamine/stderr-latest.log");
            ...
            if (stderr_log[0]) {
                ...
                if (afd >= 0) {
                    char exit_msg[128];
                    int elen = snprintf(exit_msg, sizeof(exit_msg),
                        "\n[amphetamine] killed by signal %d\n", sig);
                    write(afd, exit_msg, (size_t)elen);
                    close(afd);
                }
            }
...
int main(int argc, char *argv[]) {
    // Initialize globals
    g_verbose = getenv("AMPHETAMINE_VERBOSE") != nullptr;
