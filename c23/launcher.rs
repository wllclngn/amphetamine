...
pub fn run(verb: &str, args: &[String]) -> i32 {
    // Ensure fds 0, 1, 2 are occupied. Wine's socketpair() picks the lowest
    // available fd. If stdin is closed (common under Steam's reaper), fd 0 is
    // free and socketpair returns fd 0. The child then does set_stdio_fd which
    // overwrites fd 0 with /dev/null, destroying the WINESERVERSOCKET fd.
    ensure_stdio_fds();
...
fn setup_prefix(wine_dir: &Path, pfx: &Path, wine64: &Path, self_exe: &Path) {
    ...
    if default_pfx.exists() {
    ...
    } else if pfx.join("system.reg").exists() {
        // Prefix exists but no template — just repair dosdevices/symlinks.
        // Skip wineboot --init: triskelion doesn't implement every server
        // call wineboot needs, and the prefix is already functional.
        ...
        // No template, no existing prefix — must run wineboot
...
fn build_env_vars(
    ...
    // Display driver: winex11.drv + GLX over XWayland. No winewayland.
