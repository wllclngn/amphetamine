...
/* ── Relay (kernel↔daemon for non-sync opcodes) ───────────────────── */

/*
 * Wine issues TRISKELION_IOC_RELAY for any protocol opcode that isn't
 * handled directly in-kernel (sync, messages, wait). The kernel copies
 * the raw Wine protocol bytes, blocks the Wine thread, and wakes the
 * userspace daemon. The daemon read()s the request, processes it,
 * write()s the reply, and the kernel unblocks Wine.
 */

#define TRISKELION_RELAY_HAS_FD        0x01  /* request carries file descriptors */
#define TRISKELION_RELAY_REPLY_HAS_FD  0x02  /* reply carries file descriptors */
#define TRISKELION_RELAY_MAX_FDS       4

/* Envelope header for daemon read()/write() — prefixed to raw payload */
struct triskelion_relay_header {
	__u64 request_id;
	__u32 client_id;
	__u32 opcode;
	__u32 payload_size;     /* 64 fixed + vararg */
	__u32 flags;
	__u32 fd_count;
	__s32 fds[TRISKELION_RELAY_MAX_FDS];
};

/* Wine ioctl args: relay a protocol request to the daemon */
struct triskelion_relay_args {
	const void __user *request;     /* 64-byte fixed + vararg */
	void __user *reply;             /* reply buffer */
	__u32 request_size;             /* total request bytes */
	__u32 reply_max;                /* reply buffer capacity */
	__u32 reply_size;               /* out: actual reply size */
	__u32 flags;
	__s32 fds[TRISKELION_RELAY_MAX_FDS];
	__u32 fd_count_in;              /* in: fds sent with request */
	__u32 fd_count_out;             /* out: fds received with reply */
};

/* Daemon registration */
struct triskelion_daemon_args {
	__u32 version;                  /* protocol version (930) */
};

#define TRISKELION_IOC_RELAY           _IOWR(TRISKELION_IOCTL_MAGIC, 0x50, struct triskelion_relay_args)
#define TRISKELION_IOC_REGISTER_DAEMON _IOW (TRISKELION_IOCTL_MAGIC, 0x60, struct triskelion_daemon_args)
