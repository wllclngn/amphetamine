...
def main():
    ...
    if amp_stderr:
        amp_display = extract_display_lines(amp_stderr, [
    ...
    # ── Diff ─────────────────────────────────────────────────────────
    if amp_stderr and stock_stderr:
        print(f"\n  --- DIFF: Display Init Sequence ---")

        amp_ops = set()
        stock_ops = set()

        for line in extract_display_lines(amp_stderr, ["add_gpu", "commit_gpu", "add_source",
        ...
        only_stock = stock_ops - amp_ops
        only_amp = amp_ops - stock_ops
        both = amp_ops & stock_ops
        ...
        if only_amp:
            print(f"    AMP ONLY:   {', '.join(sorted(only_amp))}")
        if not only_stock and not only_amp:
            print(f"    No differences in display init functions called")

        # Check for window creation
        amp_window = any("RegisterTouch" in l or "create_whole" in l or "drawable_create" in l
                         for l in extract_display_lines(amp_stderr, ["RegisterTouch", "create_whole", "drawable_create"]))
